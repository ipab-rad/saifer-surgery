function mk_export_calib_world(TR, TW, TWframe, launchPath )
...
Q = rotm2quat(R); % form q = [w x y z], with w as the scalar number.
%spose = sprintf('%f %f %f %f %f %f %f',T(1),T(2),T(3),Q(2),Q(3),Q(4),Q(1)); % x y z qx qy qz qw
spose = '-0.95 -0.1 1.45 3.1 3.14139 0';  % MANUAL
fprintf(fid,'  <node pkg="tf" type="static_transform_publisher" name="world_realsense_%d_tfs" args="%s map %s 50"/>\n',5,spose,TWframe);
% static_transform_publisher x y z qx qy qz qw frame_id child_frame_id  period_in_ms
